...
#ifndef SHEPHERD_INTERFACE_DEFINITIONS_HPP
#define SHEPHERD_INTERFACE_DEFINITIONS_HPP

#include <shepherd/utils/context_propagation_definitions.hpp>
#include <cstdint>
#include <string>
...
#define REMOVE_RULE            2
#define CREATE_HSK_RULE        3
#define EXEC_HSK_RULES         4
#define CREATE_DIF_RULE        5
#define CREATE_ENF_RULE        6
#define COLLECT_DETAILED_STATS 7
#define STAGE_READY            8
#define COLLECT_STATS_KVS      9
#define COLLECT_STATS_TF       10
#define COLLECT_GLOBAL_STATS   11
#define COLLECT_ENTITY_STATS   12
#define LOCAL_HANDSHAKE        13
...
/**
 * stage_hostname_max_size: defines the maximum size of the StageInfo's hostname.
 */
const int stage_hostname_max_size = 50;

/**
 * stage_user_max_size: defines the maximum size of the StageInfo's user.
 */
const int stage_user_max_size = 50;
...
struct StageSimplifiedHandshakeRaw {
    ...
    int m_pid { -1 };
    int m_ppid { -1 };
    char m_stage_hostname[stage_hostname_max_size] {};
    char m_stage_user[stage_user_max_size] {};
};

/**
 * stage_description_max_size: defines the maximum size of the StageInfo's description.
 */
const int stage_description_max_size = 500;

/**
 * StageDetailedHandshakeRaw: Raw structure that identifies the Data Plane Stage.
 * - m_stage_name: defines the stage identifier (name).
 * - m_stage_env: defines the environment variable value registered for the data plane stage;
 * - m_stage_description: defines the stage description.
 * - m_pid: defines the pid of the process where the data plane stage is executing;
 * - m_ppid: defines the parent pid of the process where the data plane stage is executing.
 * TODO: create dedicated object.
 */
struct StageDetailedHandshakeRaw {
    char m_stage_name[stage_name_max_size] {};
    char m_stage_env[stage_env_max_size] {};
    char m_stage_description[stage_description_max_size] {};
    int m_pid { -1 };
    int m_ppid { -1 };
};
...
/**
 * EnforcementRuleWithTimeRaw: Raw structure to perform the serialization of EnforcementRules between the
 * PAIO stage and control the plane.
...
/**
 * StatsRaw: Raw structure to perform the serialization of I/O statistics between the
...
struct StatsGlobalRaw {
    struct StatsDataRaw m_data_stats;
    struct StatsMetadataRaw m_metadata_stats;
};

/**
 * StatsEntityRaw: Raw structure to perform the serialization of I/O statistics between the
 * PAIO stage and the control plane. These statistics are specific to the "Per-application bandwidth
 * control" use case of the PAIO paper, where we demonstrate how to ensure QoS bandwidth across
 * multiple applications sharing the same storage device.
 * - m_fg_tasks:
 */
 //StatsChannelRaw
struct StatsEntityRaw {
    //Deixar de enviar metadataraw e mandar apenas o total das ops da metadata
...
#endif // SHEPHERD_INTERFACE_DEFINITIONS_HPP
