...
private:
    ...
    void fill_create_channel_rule_grpc (
        controllers_grpc_interface::HousekeepingCreateChannelRaw* hsk_channel_obj,
        const std::vector<std::string>& tokens);

    void fill_create_object_rule (HousekeepingCreateObjectRaw* hsk_object_obj,
        const std::vector<std::string>& tokens);

    void fill_create_object_rule_grpc (
        controllers_grpc_interface::HousekeepingCreateObjectRaw* hsk_object_obj,
        const std::vector<std::string>& tokens);
...
public:
     ...
     * @return PStatus value that defines if the operation was successful.
     */
    PStatus RemoveRule (const std::string& user_address,
        ControlOperation* operation,
        int rule_id,
        ACK& response);
